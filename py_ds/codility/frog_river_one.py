...
leaf falls at time K, measured in seconds. The goal is to find the earliest time when the frog can jump to the other side
of the river. The frog can cross only when leaves appear at every position across the river from 1 to X
(that is, we want to find the earliest moment when all the positions from 1 to X are covered by leaves).
You may assume that the speed of the current in the river is negligibly small, i.e. the leaves do not change their positions
...
def solution(x: int, a: List[int]) -> int:
    ...
    highest = (0, a[0])
    for elem in [x for x in enumerate(a)]:
        if elem[1] > highest[1]:
            highest = elem
    return highest[0]
...
if __name__ == '__main__':
    ...
    max = 100 * 1000
    answer = solution(max, list([x for x in range(1, max)] + [max]))
    assert answer == (max - 1)